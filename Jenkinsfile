...
exist=`aws cloudformation list-stacks --stack-status-filter CREATE_IN_PROGRESS CREATE_COMPLETE ROLLBACK_IN_PROGRESS ROLLBACK_FAILED ROLLBACK_COMPLETE DELETE_IN_PROGRESS DELETE_FAILE
...
network=`aws cloudformation list-stacks --stack-status-filter CREATE_COMPLETE ROLLBACK_COMPLETE UPDATE_COMPLETE_CLEANUP_IN_PROGRESS UPDATE_COMPLETE UPDATE_ROLLBACK_COMPLETE --query
...
aws cloudformation describe-stacks --stack-name project2-lb | jq ".Stacks" | 




jq -r \'.[].Outputs[].OutputValue\' | grep http
'''
      }
    }
